...
2.1) The `addWorkout` function should be used to add a workout session for a given member ID. If the workout is stored successfully, the function should return `true`. If the given member does not exist while calling this function, the workout should be ignored and the function should return `false`.
...
*/
import java.util.*;
// JUnit is not required for these simple in-file tests; provide minimal helpers below.
...
class Membership {
  ...
  */
  ...
  // Map from memberId -> list of workouts for that member
  ...
  public void addMember(Member member) {
    members.add(member);
    // Ensure an entry exists in the workouts map for every member
    workouts.putIfAbsent(member.memberId, new ArrayList<>());
  }
  ...
  public boolean addWorkout(int memberId, Workout workout) {
    // If the member does not exist, ignore the workout and return false
    if (!workouts.containsKey(memberId)) {
      return false;
    }

    // Add the workout to the member's workout list
    workouts.get(memberId).add(workout);
    return true;
  }
  ...
  public Map<Integer, Double> getAverageWorkoutDurations() {
    ...
    for (Member member : members) {
      List<Workout> wlist = workouts.get(member.memberId);
      ...
      averages.put(member.memberId, sum / wlist.size());
    }
...
public class Solution {
  ...
  */
  ...
  // Minimal test helpers to avoid requiring JUnit on the classpath
